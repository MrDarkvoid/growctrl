...
`,O={ok:{bg:"rgba(77,255,195,.14)",color:u.ok,label:"Alles OK"},info:{bg:"rgba(126,200,255,.14)",color:u.info,label:"Info"},warning:{bg:"rgba(255,209,102,.16)",color:u.warn,label:"Warnung"},critical:{bg:"rgba(255,107,107,.18)",color:u.crit,label:"Fehler"}};var D=30,ft=4,Pe=6,Rt=14,qe=0;function Xe(n){if(n.length<3)return`M${n.map(e=>e.join(",")).join(" L")}`;let t=`M${n[0][0]},${n[0][1]}`;for(let e=0;e<n.length-1;e++){let s=n[Math.max(0,e-1)],i=n[e],r=n[e+1],o=n[Math.min(n.length-1,e+2)],c=i[0]+(r[0]-s[0])/6,a=i[1]+(r[1]-s[1])/6,h=r[0]-(o[0]-i[0])/6,g=r[1]-(o[1]-i[1])/6;t+=` C${c.toFixed(1)},${a.toFixed(1)} ${h.toFixed(1)},${g.toFixed(1)} ${r[0]},${r[1]}`}return t}function H(n,t={}){let e=`gcg${qe++}`,s=t.w??300,i=t.h??110,r=n.flatMap(m=>m.data);if(!r.length)return l`<div style="height:${i}px;display:flex;align-items:center;justify-content:center;
    ...
    ${n.map((m,y)=>{if(m.data.length<2)return d;let v=m.data.map((z,b)=>[Number(h(b,m.data.length).toFixed(1)),Number(g(z).toFixed(1))]),L=Xe(v),E=v[v.length-1][0],P=v[v.length-1][1];return ot`
        <defs>
  ...
  <div style="display:flex;gap:12px;flex-wrap:wrap;margin-top:4px">
    ${n.filter(t=>t.name).map(t=>l`<span style="display:inline-flex;align-items:center;gap:5px;
    ...
    </div>`}render(){let e=this._config;if(!this.hass)return d;let s=this.hass.states[this.e("vpd")],i=!s&&!this.hass.states[this.e("enabled")],r=A(s?.state)??(i?1.06:null),o=s?.attributes?.temp,c=s?.attributes?.rh,a=s?.attributes?.phase_effektiv??"Veg",h=s?.attributes?.sollwerte,g=this.isOn(this.e("enabled")),f=this.isOn(this.e("climate")),_=this.hass.states[this.e("status")],m=_?.attributes?.probleme??[],y=_?.state?.toLowerCase?.()==="problem"?"warning":g?"ok":"none",v=O[y]??O.none,L=this.hass.states[this.e("event")],E=r!==null&&h&&r>=h.vpd_min&&r<=h.vpd_max,P=(b,T,C,k)=>l`
      ...
      <div class="tile" style="text-align:center">
        <div class="lbl">${b}</div>
        <div class="val" style="font-size:22px;${k?`color:${k}`:""}">${T}<span class="unit">${C}</span></div>
      ...
      <div class="kpis cols-3" style="margin-top:10px">
        ${z("Temperatur",o!=null?Number(o).toFixed(1):"\u2013","\xB0C")}
        ${z("Luftfeuchte",c!=null?String(Math.round(Number(c))):"\u2013","%")}
        ${z("VPD",r!==null?r.toFixed(2):"\u2013","kPa",r===null?void 0:E?u.ok:"#FFD166")}
      </div>
      ...
      <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:12px">
        ${Ye.map(k=>{let U=mt[k],K=k===r;return l`<button class="gc" style="padding:5px 12px;border-radius:10px;font-size:11px;
              font-weight:700;transition:all .15s;
      ...
      ${b>0?l`<div class="kpis cols-${Math.min(3,Math.max(2,b))}" style="margin-top:10px">
        ${P?T("Pumpe",s?"12 min":zt(Number(this.st(this.e("pumpRest")))),s?"Restzeit (Demo)":"Restzeit"):d}
        ${z?T("DLI heute",m!==null?m.toFixed(1):"\u2013",v?`Ziel ${v} \xB7 Prognose ${y!==null?y.toFixed(1):"\u2013"}`:void 0,v&&m!==null&&m>=v?u.ok:void 0):d}
      ...
      ${this.renderConfirm()}
    </div>`}lightRow(){if(this.isPreview)return this.lightRowView(!0,"Licht an f\xFCr 5 h 40 min",.62);let e=this.hass.states[this.e("lightRest")],s=Number(e?.state),i=e?.attributes??{},r=i.zustand?i.zustand==="an":void 0,o=i.text??(isNaN(s)?"\u2013":`Restzeit ${zt(s)}`),c=typeof i.anteil=="number"?Math.min(1,Math.max(0,i.anteil)):null;return this.lightRowView(r,o,c)}lightRowView(e,s,i){let r=e===!1?"#7A8CA8":"#FFD166";return l`<div class="tile" style="margin-top:12px;display:flex;align-items:center;gap:12px">
      ...
      <div class="tile" style="margin-top:8px">
        <div style="display:flex;align-items:center;gap:12px">
          ${i.image?l`<img src="${i.image}" style="width:52px;height:52px;border-radius:12px;
              object-fit:cover;border:1px solid rgba(255,255,255,.15)"/>`:d}
          <div style="flex:1;min-width:0">
            <div style="font-size:14px;font-weight:800">${i.name}</div>
            <div style="font-size:11px;color:rgba(255,255,255,.6)">
              ${i.strain??""}${i.strain&&o!==null?" \xB7 ":""}${o!==null?`${at(o)} \xB7 Tag ${o+1}`:""}</div>
          </div>
        </div>
        ${i.sensors?.length?l`
          <div class="kpis cols-${Math.min(3,Math.max(2,i.sensors.length))}" style="margin-top:10px">
            ${i.sensors.map(c=>{let a=A(this.st(c));return l`<div class="tile" style="background:rgba(0,0,0,.18)">
                <div class="lbl">${this.friendly(c)}</div>
                <div style="font-size:18px;font-weight:800">${a!==null?a:this.st(c)??"\u2013"}
                  <span class="unit">${this.unit(c)}</span></div>
              </div>`})}
          </div>`:d}
      ...
      <div class="grid" style="grid-template-columns:repeat(${s},1fr)">
        ${e.sensors.map(r=>{let o=A(this.st(r.entity)),c=this.bad(o,r),a=Le(this._hist[r.entity]??[],100,26),h=r.name??this.friendly(r.entity);return l`<button class="gc tile" style="text-align:left;position:relative;overflow:hidden;
      ...
      <div class="grid" style="grid-template-columns:repeat(${e.columns??2},1fr)">
        ${e.plants.map(s=>{let i=s.germination_helper?this.st(s.germination_helper):void 0,r=i?ut(i):null,o=s.camera?this.hass.states[s.camera]?.attributes?.entity_picture?`${this.hass.states[s.camera].attributes.entity_picture}&t=${this._tick}`:void 0:s.image;return l`<div class="tile" style="overflow:hidden">
            ...
            ${r!==null?l`<div class="lbl" style="margin-top:4px;color:rgba(255,255,255,.7)">${at(r)} \u00b7 Tag ${r+1}</div>`:d}
      ...
      ${this.styleSection()}`}};customElements.define("growctrl-hero-editor",Kt);var qt=class extends x{constructor(){super(...arguments);this._hist=[]}static{this.styles=S}static{this.properties={...x.properties,_hist:{state:!0}}}validateConfig(e){if(!e.tent)throw new Error("growctrl-hero-card: 'tent' ist Pflicht (Zelt-Name aus der Integration).")}static getConfigElement(){return document.createElement("growctrl-hero-editor")}static getStubConfig(){return{tent:"gross",stations:[{station:"main1"}]}}te(e){let[s,i,r]=yt[e],o=this._config;return o.overrides?.[i]??Z(this.hass,o.tent,"zelt",r)??xt(s,o.tent,i,o.overrides)}connectedCallback(){super.connectedCallback(),this._load(),this._timer=window.setInterval(()=>this._load(),5*6e4)}disconnectedCallback(){super.disconnectedCallback(),this._timer&&clearInterval(this._timer)}async _load(){if(!this.hass){setTimeout(()=>this._load(),1e3);return}this._hist=await M(this.hass,this.te("vpd"),this._config.hours??24)}render(){let e=this._config;if(!this.hass)return d;let s=this.hass.states[this.te("vpd")],i=!s&&!this.hass.states[this.te("enabled")],r=A(s?.state)??(i?1.06:null),o=s?.attributes?.temp??(i?22.4:null),c=s?.attributes?.rh??(i?52:null),a=s?.attributes?.phase_effektiv??"",h=s?.attributes?.sollwerte,g=this.isOn(this.te("enabled"))||i,f=this.isOn(this.te("climate")),_=this.hass.states[this.te("status")],m=_?.attributes?.probleme??[],y=(e.stations??[]).map(b=>{let T=this.hass.states[Z(this.hass,e.tent,b.station,"last_event")??bt("sensor",e.tent,b.station,"letztes_ereignis",e.overrides)],C=T?.attributes?.schweregrad??"ok";return{name:b.name??b.station,text:T?.state??"\u2013",level:C}}),v=b=>b==="warning"||b==="critical",L=Q([(_?.state??"").toLowerCase()==="problem"?"warning":"ok",...y.map(b=>v(b.level)?b.level:"ok")]),E=O[L],P=[...m.map(b=>({label:b,level:"warning"})),...y.filter(b=>v(b.level)).map(b=>({label:`${b.name}: ${b.text}`,level:b.level}))],z=(b,T,C,k)=>l`
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:13px">
        ${z(this.te("enabled"),"Zelt",g,"mdi:power")}
        ${z(this.te("climate"),"Klima",f,"mdi:thermostat")}
      </div>
