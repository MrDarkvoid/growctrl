...
`,M={ok:{bg:"rgba(123,232,168,.14)",color:h.ok,label:"Alles OK"},info:{bg:"rgba(154,200,255,.14)",color:h.info,label:"Info"},warning:{bg:"rgba(255,206,122,.14)",color:h.warn,label:"Warnung"},critical:{bg:"rgba(255,157,157,.16)",color:h.crit,label:"Kritisch"}};var B=30,ut=4,Se=6,Lt=14,Ce=0;function Ae(o){if(o.length<3)return`M${o.map(t=>t.join(",")).join(" L")}`;let e=`M${o[0][0]},${o[0][1]}`;for(let t=0;t<o.length-1;t++){let n=o[Math.max(0,t-1)],i=o[t],r=o[t+1],s=o[Math.min(o.length-1,t+2)],c=i[0]+(r[0]-n[0])/6,a=i[1]+(r[1]-n[1])/6,d=r[0]-(s[0]-i[0])/6,g=r[1]-(s[1]-i[1])/6;e+=` C${c.toFixed(1)},${a.toFixed(1)} ${d.toFixed(1)},${g.toFixed(1)} ${r[0]},${r[1]}`}return e}function D(o,e={}){let t=`gcg${Ce++}`,n=e.w??300,i=e.h??110,r=o.flatMap(f=>f.data);if(!r.length)return l`<div style="height:${i}px;display:flex;align-items:center;justify-content:center;
    ...
    ${o.map((f,v)=>{if(f.data.length<2)return p;let y=f.data.map((R,b)=>[Number(d(b,f.data.length).toFixed(1)),Number(g(R).toFixed(1))]),E=Ae(y),A=y[y.length-1][0],P=y[y.length-1][1];return st`
        <defs>
  ...
  <div style="display:flex;gap:12px;flex-wrap:wrap;margin-top:4px">
    ...
    <div style="display:flex;justify-content:space-between;font-size:9.5px;
    ...
    </div>`}render(){let t=this._config;if(!this.hass)return p;let n=this.hass.states[this.e("vpd")],i=!n&&!this.hass.states[this.e("enabled")],r=S(n?.state)??(i?1.06:null),s=n?.attributes?.temp,c=n?.attributes?.rh,a=n?.attributes?.phase_effektiv??"Veg",d=n?.attributes?.sollwerte,g=this.isOn(this.e("enabled")),m=this.isOn(this.e("climate")),x=this.hass.states[this.e("status")],f=x?.attributes?.probleme??[],v=x?.state?.toLowerCase?.()==="problem"?"warning":g?"ok":"none",y=M[v]??M.none,E=this.hass.states[this.e("event")],A=r!==null&&d&&r>=d.vpd_min&&r<=d.vpd_max,P=(b,C,F,W)=>l`
      <button class="gc" style="flex:1;display:flex;align-items:center;justify-content:center;gap:7px;
          ...
          background:${F?"rgba(77,255,195,.12)":"rgba(255,255,255,.05)"};
          border:1.5px solid ${F?h.ok:"rgba(255,255,255,.12)"};
          color:${F?h.ok:"rgba(255,255,255,.5)"}"
        @click=${()=>this.confirmToggle(b,C)}>
        <ha-icon .icon=${W} style="--mdc-icon-size:15px"></ha-icon>
        <span style="font-size:11px;font-weight:800">${C} ${F?"AN":"AUS"}</span>
      </button>`,R=(b,C,F,W)=>l`
      <div class="tile" style="text-align:center">
        <div class="lbl">${b}</div>
        <div class="val" style="font-size:22px;${W?`color:${W}`:""}">${C}<span class="unit">${F}</span></div>
      ...
      <div class="kpis cols-3" style="margin-top:10px">
        ${R("Temperatur",s!=null?Number(s).toFixed(1):"\u2013","\xB0C")}
        ${R("Luftfeuchte",c!=null?String(Math.round(Number(c))):"\u2013","%")}
        ${R("VPD",r!==null?r.toFixed(2):"\u2013","kPa",r===null?void 0:A?h.ok:"#FFD166")}
      </div>
      ...
      ${t.show_chart!==!1&&this._hist.length>1?l`
        ...
        ${D([{data:this._hist,color:A===!1?"#FFD166":h.ok}],{w:this.chartW(),h:100,band:d?{min:d.vpd_min,max:d.vpd_max}:void 0,grid:3})}`:p}
      ...
      ${E?l`<button class="gc logrow" style="width:100%;margin-top:9px;text-align:left"
          ...
          <span class="dot" style="background:${L[E.attributes?.schweregrad]??h.info};flex-shrink:0"></span>
          <span class="txt" style="color:rgba(255,255,255,.6)">${E.state}</span>
        </button>`:p}
      ${this.renderConfirm()}
    </div>`}};customElements.define("growctrl-tent-card",Pt);var Rt=class extends _{render(){let e=[this.tentSelect(),this.stationSelect(this._config?.tent),u.text("name","Anzeigename (optional)"),u.bool("show_settings","Einstellungen-Zahnrad anzeigen"),u.bool("show_event","Ereignisfeld am Kartenfuss (Standard an)"),u.entity("tank_entity","Stations-Tank F\xFCllstand % (optional)","sensor"),u.num("tank_min","Tank-Mindeststand %",0,100),u.num("tank_volume","Tank-Volumen in Litern (optional)",1,1e4)],t=[u.text("name","Name"),u.text("strain","Sorte (optional)"),u.entity("germination_helper","Keimstart-Entity (date, optional)","date"),u.entities("sensors","Sensoren dieser Pflanze (optional)","sensor"),u.text("image","Bild-URL (optional)"),u.entity("tank_entity","Tank-F\xFCllstand in % (optional)","sensor"),u.num("tank_min","Tank-Mindeststand % (Standard 30)",0,100)];return l`${this.form(e)}
      ...
      <div class="hdr">
        <div style="min-width:0">
          ...
          <div class="subtitle" style="display:flex;align-items:center;gap:6px">
            ...
            ${c?l`<span style="color:${h.warn};font-weight:800">\u00b7 Wartung</span>`:p}
          </div>
        </div>
      ...
      ${t.show_event!==!1&&d?l`
        <button class="gc event" @click=${()=>this.moreInfo(this.e("event"))}>
          <span class="edot" style="background:${L[d.attributes?.schweregrad]??"rgba(242,247,243,.4)"}"></span>
          <span class="ebody">
      ...
      <div class="plant">
        <div class="phd">
          ...
          <div style="flex:1;min-width:0">
            ...
            ${i.strain?l`<div class="pstrain">${i.strain}</div>`:p}
      ...
      </div>`}plantSensors(t){if(!t.length)return p;let n=t.map(i=>typeof i=="string"?{entity:i}:i);return l`${n.map(i=>this.sensorInd(i))}`}sensorInd(t){let n=S(this.st(t.entity)),i=t.name??this.friendly(t.entity),r=this.unit(t.entity),s=t.anzeige??"wert",c=t.entity.split(".")[0],a=c==="number"||c==="input_number",d=this.hass.states[t.entity]?.attributes??{},g=t.step??(Number(d.step)||.1),m=d.min,x=d.max,f=(String(g).split(".")[1]??"").length||1,v=b=>{let C=b;m!==void 0&&(C=Math.max(m,C)),x!==void 0&&(C=Math.min(x,C)),this.hass.callService(c,"set_value",{entity_id:t.entity,value:Number(C.toFixed(f))})},y,E=t.ideal??[0,0],A=t.ok??E;if(s==="zone"){let b=n!==null&&n>=E[0]&&n<=E[1],C=n!==null&&n>=A[0]&&n<=A[1];y=t.color??(b?h.ok:C?h.warn:h.crit)}else s==="graph"?y=t.color??h.water:y=t.color??"rgba(242,247,243,.95)";let P=l`<div class="ihd">
      <span class="ilbl" style="color:${s==="wert"?"rgba(242,247,243,.62)":y};cursor:pointer"
        ...
        ${a?l`<ha-icon icon="mdi:pencil" style="--mdc-icon-size:11px;opacity:.45;margin-left:3px"></ha-icon>`:p}
      </span>
    ...
    </div>`,R=s==="zone"?l`<div style="margin-top:8px">${Te(n,{min:t.min??0,max:t.max??14,okMin:A[0],okMax:A[1],idealMin:E[0],idealMax:E[1]})}</div>`:s==="graph"?l`<div style="margin-top:6px">${Fe(this._spark[t.entity]??[],y,this.chartW(74),38)}</div>`:p;return l`<div class="ind">${P}${R}</div>`}plantTankInd(t,n){let i=Math.min(100,Math.max(0,S(this.st(t))??0)),s=i<n?h.crit:h.water;return l`<button class="gc ind" @click=${()=>this.moreInfo(t)}>
    ...
    </div>`}};customElements.define("growctrl-sensors-card",Bt);var nn=[u.text("title","Titel"),u.num("limit","Max. Zeilen",3,50),u.select("min_level","Anzeige",[{value:"alle",label:"Alle Ereignisse"},{value:"warnung",label:"Nur Warnungen/Fehler"}])],sn=[u.entity("entity","Letztes-Ereignis-Sensor","sensor"),u.text("name","Label (optional)")],Dt=class extends _{render(){return l`${this.form(nn)}
      ${this.list({key:"sources",rowSchema:sn,title:"Quellen",addLabel:"Quelle hinzuf\xFCgen",newItem:()=>({entity:""})})}
      ${this.styleSection()}`}};customElements.define("growctrl-status-editor",Dt);var Ut=class extends ${static{this.styles=k}validateConfig(e){if(!Array.isArray(e.sources)||!e.sources.length)throw new Error("growctrl-status-card: 'sources' (Letztes-Ereignis-Sensoren) ist Pflicht.")}static getConfigElement(){return document.createElement("growctrl-status-editor")}static getStubConfig(){return{sources:[{entity:"sensor.growctrl_gross_main1_letztes_ereignis"}]}}render(){let e=this._config;if(!this.hass)return p;let t=[],n=[];for(let a of e.sources){let d=this.hass.states[a.entity],g=d?.attributes?.verlauf??[];n.push(d?.attributes?.schweregrad??"ok"),g.forEach(m=>t.push({...m,src:a.name??this.friendly(a.entity)}))}t.reverse();let r=(e.min_level==="warnung"?t.filter(a=>a.level==="warning"||a.level==="critical"):t).slice(0,e.limit??12),s=q(n.map(a=>a==="ok"?"ok":a)),c=M[s];return l`<div class="card ${e.style?.glass?"glass":""}" data-level=${s} style=${w(e.style)}>
      <div class="hdr">
      ...
      <div style="margin-top:9px">
        ${r.length?r.map(a=>l`
          <div class="logrow" style="background:${a.level==="info"?"transparent":N[a.level]??"transparent"};
            ...
            <span class="txt" style="color:${a.level==="info"?"rgba(255,255,255,.6)":L[a.level]??"rgba(255,255,255,.6)"}">${a.text}</span>
          </div>`):l`<div class="logrow"><span class="txt" style="color:${h.ok}">
            \u2713 Noch keine Ereignisse</span></div>`}
      </div>
    </div>`}};customElements.define("growctrl-status-card",Ut);var rn=[u.text("title","Titel (optional)"),u.text("logo","Logo-URL (z.B. /local/growctrl/logo.png)"),u.bool("show_chart","24h-Chart zus\xE4tzlich zum Zonen-Balken anzeigen"),u.num("hours","Chart-Zeitraum (h)",1,168)],Ht=class extends _{render(){let e=[this.stationSelect(this._config?.tent),u.text("name","Label (optional)")];return l`${this.form([this.tentSelect(),...rn])}
      ${this.list({key:"stations",rowSchema:e,title:"Stationen (Informationssystem)",addLabel:"Station hinzuf\xFCgen",newItem:()=>({station:""})})}
      ${this.styleSection()}`}};customElements.define("growctrl-hero-editor",Ht);var Gt=class extends ${constructor(){super(...arguments);this._logoErr=!1;this._hist=[]}static{this.styles=k}static{this.properties={...$.properties,_hist:{state:!0},_logoErr:{state:!0}}}validateConfig(t){if(!t.tent)throw new Error("growctrl-hero-card: 'tent' ist Pflicht (Zelt-Name aus der Integration).")}static getConfigElement(){return document.createElement("growctrl-hero-editor")}static getStubConfig(){return{tent:"gross",stations:[{station:"main1"}]}}te(t){let[n,i,r]=mt[t],s=this._config;return s.overrides?.[i]??j(this.hass,s.tent,"zelt",r)??gt(n,s.tent,i,s.overrides)}connectedCallback(){super.connectedCallback(),this._load(),this._timer=window.setInterval(()=>this._load(),5*6e4)}disconnectedCallback(){super.disconnectedCallback(),this._timer&&clearInterval(this._timer)}async _load(){if(!this.hass){setTimeout(()=>this._load(),1e3);return}this._hist=await T(this.hass,this.te("vpd"),this._config.hours??24)}render(){let t=this._config;if(!this.hass)return p;let n=this.hass.states[this.te("vpd")],i=!n&&!this.hass.states[this.te("enabled")],r=S(n?.state)??(i?1.06:null),s=n?.attributes?.temp??(i?22.4:null),c=n?.attributes?.rh??(i?52:null),a=n?.attributes?.phase_effektiv??"",d=n?.attributes?.sollwerte,g=this.isOn(this.te("enabled"))||i,m=this.isOn(this.te("climate")),x=this.hass.states[this.te("status")],f=x?.attributes?.probleme??[],v=(t.stations??[]).map(b=>{let C=this.hass.states[j(this.hass,t.tent,b.station,"last_event")??ht("sensor",t.tent,b.station,"letztes_ereignis",t.overrides)],F=C?.attributes?.schweregrad??"ok";return{name:b.name??b.station,text:C?.state??"\u2013",level:F}}),y=b=>b==="warning"||b==="critical",E=q([(x?.state??"").toLowerCase()==="problem"?"warning":"ok",...v.map(b=>y(b.level)?b.level:"ok")]),A=M[E],P=[...f.map(b=>({label:b,level:"warning"})),...v.filter(b=>y(b.level)).map(b=>({label:`${b.name}: ${b.text}`,level:b.level}))],R=(b,C,F,W)=>l`
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:13px">
        ...
        ${R(this.te("climate"),"Klima",m,"mdi:thermostat")}
      </div>
      ...
      <div class="kpis cols-3" style="margin-top:12px">
        ...
        <div class="tile" style="text-align:center"><div class="lbl">VPD</div>
          <div class="val" style="font-size:22px;color:${r!==null&&d&&r>=d.vpd_min&&r<=d.vpd_max?h.ok:"#FFD166"}">${r!==null?r.toFixed(2):"\u2013"}<span class="unit">kPa</span></div></div>
      </div>
      ...
      ${t.show_chart===!0&&this._hist.length>1?l`
        <div class="seclbl">VPD \u00b7 ${t.hours??24}h</div>
        ${D([{data:this._hist,color:h.ok}],{w:this.chartW(),h:100,band:d?{min:d.vpd_min,max:d.vpd_max}:void 0,grid:3})}`:p}

      ${v.length?l`<div class="seclbl">Stationen</div>
        ${v.map(b=>l`<div class="logrow" style="margin-top:3px;padding:6px 9px">
          <span class="dot" style="background:${b.level==="ok"?h.ok:L[b.level]??h.warn};flex-shrink:0"></span>
      ...
      ${P.length?P.map(b=>l`<div class="logrow" style="background:${N[b.level]??N.warning};margin-top:4px">
            <span class="txt" style="color:${L[b.level]??L.warning}">\u26A0 ${b.label}</span></div>`):l`<div class="logrow" style="background:${N.ok};margin-top:4px">
      ...
      <div style="position:relative;height:12px;border-radius:6px;
        ...
        ${i.map(a=>{let d=(a.to-c)/r*100,g=c===0;return c=a.to,l`<div style="width:${d}%;background:${a.col};opacity:.75;
